...
new = function ( params )
   ...
   --Btn Score
	...
	--Btn monScore
	local btnMonScore = display.newRect(0, 0, 100, 30)
	btnMonScore.strokeWidth = 1
	
	btnMonScore:setStrokeColor(0, 0, 0)
	btnMonScore:setReferencePoint( display.TopLeftReferencePoint )
	btnMonScore.x = 20
	btnMonScore.y = 245
	localGroup:insert(btnMonScore)
	   
	text = display.newText("Mon score", 0, 0, "Arial", 14)
	text:setReferencePoint( display.CenterLeftReferencePoint )
	text:setTextColor(255, 255, 255)
	text.x = 30
	text.y = 260 
   ...
   --Btn Global
	local btnGlobal = display.newRect(0, 0, 100, 30)
	btnGlobal.strokeWidth = 1
	
	btnGlobal:setStrokeColor(0, 0, 0)
	btnGlobal:setReferencePoint( display.TopRightReferencePoint )
	btnGlobal.x = display.contentWidth - 20
	btnGlobal.y = 245
	localGroup:insert(btnGlobal)
	   
	text = display.newText("Global", 0, 0, "Arial", 14)
	text:setReferencePoint( display.CenterLeftReferencePoint )
	text:setTextColor(255, 255, 255)
	text.x = display.contentWidth - 10 - btnGlobal.width
	text.y = 260 
   ...
   --Gestion de l'apparence de l'onglet actif
	local function apparenceOngletActif ()
		...
		btnGlobal:setFillColor(0, 56, 112, 100)
		btnMonScore:setFillColor(0, 56, 112, 100)
		if(afficheScore.monScore) then btnMonScore:setFillColor(0, 56, 112) end
		if(not afficheScore.monScore) then btnGlobal:setFillColor(0, 56, 112) end
    ...
    local function pressGlobal (event)
		if event.phase == "ended" then
			score.setToMonScore(false)
			apparenceOngletActif ()
		end
	end
	btnGlobal:addEventListener ("touch", pressGlobal)
	local function pressMonScore (event)
		if event.phase == "ended" then
			score.setToMonScore(true)
			apparenceOngletActif ()
		end
	end
	btnMonScore:addEventListener ("touch", pressMonScore)
